...
-(void)addUsersToGroup:(NSMutableArray *)users
{
    ...
    [addMember requestWithObject:@[self.sessionID,userIDs] Completion:^(NSMutableArray * response, NSError *error) {
        if (response != nil) {
            ...
            [[DDDatabaseUtil instance] updateRecentGroup: self.editControll.group completion:^(NSError *error) {
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) {
        ...
        if(tf.text.length !=0)
        {
            ...
            [creatGroup requestWithObject:array Completion:^(GroupEntity * response, NSError *error) {
                if (response !=nil) {
...
//                    [[ChattingMainViewController shareInstance].module.showingMessages removeAllObjects];
                    ...
                    [[SessionModule sharedInstance] addToSessionModel:session];
                    if ([SessionModule sharedInstance].delegate && [[SessionModule sharedInstance].delegate respondsToSelector:@selector(sessionUpdate:Action:)]) {
                        [[SessionModule sharedInstance].delegate sessionUpdate:session Action:ADD];
                    }
