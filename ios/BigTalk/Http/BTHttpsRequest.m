...
+(SecIdentityRef)identityWithTrust
{
    ...
    // 证书文件名和文件类型
    NSData *PKCS12Data = [NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"yunwei" ofType:@"p12"]];
...
@implementation BTOPURLProtocal
{
    //可以在此定义数据容器，连接等
}
...
- (void)connection:(NSURLConnection *)connection didReceiveAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge
{
    ...
    if ([authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
        ...
        credential = [NSURLCredential credentialWithIdentity:[BTHttpsRequest identityWithCert] certificates:nil persistence:NSURLCredentialPersistencePermanent]; //到呢一步，certificates需要喺nil先过到
    }
