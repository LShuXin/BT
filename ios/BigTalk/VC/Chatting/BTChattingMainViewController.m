...
-(UITableViewCell *)p_promptCell_tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath message:(BTPromptEntity *)prompt;
...
-(void)sendImageMessage:(BTPhoto *)photo Image:(UIImage *)image
{
    ...
    NSDictionary *messageContentDic = @{kBTImageLocal: photo.localPath};
    ...
    [[BTDatabaseUtil instance] insertMessages:@[message] success:^{
        BTLog(@"insert image message into db successfull");
    } failure:^(NSString *errorDescripe) {
        BTLog(@"insert image message into db failed");
    }];
    photo = nil;
    [[BTSendPhotoMessageAPI sharedPhotoCache] uploadImage:messageContentDic[kBTImageLocal] success:^(NSString *imageURL) {
        [self scrollToBottomAnimated:YES];
...
-(void)textViewEnterSend
{
    ...
    BTMessageContentType msgContentType = MSG_TYPE_TEXT;
    BTMessageEntity* message = [BTMessageEntity makeMessage:text withChatModule:self.module msgType:msgContentType];
    [self.chatInputView.textView setText:nil];
    [[BTDatabaseUtil instance] insertMessages:@[message] success:^{
        BTLog(@"Insert Text Message Into DB Successfully.");
    } failure:^(NSString *errorDescripe) {
        BTLog(@"Insert Text Message Into DB Failed.");
    }];
    [self sendMessage:text messageEntity:message];
}

-(void)sendMessage:(NSString *)msg messageEntity:(BTMessageEntity *)message
{
...
-(void)p_clickThRecordButton:(UIButton *)button {
    BTLog(@"Send Voice Message Begin...");
};
...
-(void)recordingFinishedWithFileName:(NSString *)filePath time:(NSTimeInterval)interval
{
    ...
    if (length < 1)
    {
        BTLog(@"record duration is too short");
        dispatch_async(dispatch_get_main_queue(), ^{
    ...
    // append the binary data of the recording duration to the end of the recording data.
    int8_t ch[4];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [self scrollToBottomAnimated:YES];
        [[BTDatabaseUtil instance] insertMessages:@[message] success:^{
            NSLog(@"Recording Message Insert into DB Successfully.");
        } failure:^(NSString *errorDescripe) {
            NSLog(@"Recording Message Insert into DB Failed.");
        }];
    ...
    [[BTMessageSendManager instance] sendVoiceMessage:muData
        ...
        if (!error)
        {
            BTLog(@"Send Voice Message Successfully.");
...
//                    [_tableView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:index inSection:0]] withRowAnimation:UITableViewRowAnimationNone];
        ...
        {
            BTLog(@"Send Voice Message Failed.");
            message.state = MSG_SEND_FAILURE;
            [[BTDatabaseUtil instance] updateMessage:message completion:^(BOOL result) {
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(checkSessionLastMsgIDThenUpdate:) name:@"ChattingSessionUpdate" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reloginSuccess) name:BTNotificationUserReloginSuccess object:nil];
...
{
    BTMessageContentType msgContentType = MSG_TYPE_EMOTION;
    BTMessageEntity *message = [BTMessageEntity makeMessage:string withChatModule:self.module msgType:msgContentType];
    [self.chatInputView.textView setText:nil];
    [[BTDatabaseUtil instance] insertMessages:@[message] success:^{
        BTLog(@"消息插入DB成功");
    } failure:^(NSString *errorDescripe) {
...
{
    BTLog(@"prepare to load messages for session, sessionId: %@", session.sessionId);
    ...
    [self.module loadMoreHistoryCompletion:^(NSUInteger addcount, NSError *error) {
        BTLog(@"history messages for session(%@) loaded, count: %d", session.sessionId, addcount);
                                  ...
                                  message:(BTMessageEntity *)message
{
    ...
    [(BTChatVoiceCell*)cell setEarphonePlay:^{
        ...
        [[BTDatabaseUtil instance] updateMessage:message completion:^(BOOL result) {
            BTLog(@"Voice Message EarphonePlayed、DB Updated.");
        }];
    }];
    ...
    [(BTChatVoiceCell *)cell setSpeakerPlay:^{
        ...
        [weakCell showVoicePlayed];
        [[BTDatabaseUtil instance] updateMessage:message completion:^(BOOL result) {
            BTLog(@"Voice Message SpeakedPlayed、DB Updated.");
        }];
    }];
                                    ...
                                    message:(BTMessageEntity *)message
{
    ...
    [cell setTapInBubble:^{
        BTLog(@"Emotion Cell Tapped On Bubble.");
    }];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([keyPath isEqualToString:@"_inputViewY"])
    {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            if (_bottomShowComponent & BTShowEmotion)
            {
                [self.emotions.view setTop:self.chatInputView.bottom];
            }
            if (_bottomShowComponent & BTShowUtility)
            {
...
-(void)showUtilitys
{
    ...
    _recordButton.tag = INPUT_TYPE_VOICE;
    [self.chatInputView willBeginInput];
    if ([_currentInputContent length] > 0)
    {
        [self.chatInputView.textView setText:_currentInputContent];
    ...
    if (_bottomShowComponent & BTShowKeyboard)
    {
        //显示的是键盘,这是需要隐藏键盘，显示插件，不需要动画
        _bottomShowComponent = (_bottomShowComponent & 0) | BTShowUtility;
        [self.chatInputView.textView resignFirstResponder];
        [self.chatUtility.view setFrame:BTUtilityFrame];
        [self.emotions.view setFrame:BTComponentBottom];
    }
    else if (_bottomShowComponent & BTShowUtility)
    {
        //插件面板本来就是显示的,这时需要隐藏所有底部界面
//        [self p_hideBottomComponent];
        [self.chatInputView.textView becomeFirstResponder];
        _bottomShowComponent = _bottomShowComponent & BTHideUtility;
    }
    else if (_bottomShowComponent & BTShowEmotion)
    {
        //显示的是表情，这时需要隐藏表情，显示插件
        [self.emotions.view setFrame:BTComponentBottom];
        [self.chatUtility.view setFrame:BTUtilityFrame];
        _bottomShowComponent = (_bottomShowComponent & BTHideEmotion) | BTShowUtility;
    }
    else
    {
        //这是什么都没有显示，需用动画显示插件
        _bottomShowComponent = _bottomShowComponent | BTShowUtility;
        [UIView animateWithDuration:0.25 animations:^{
            [self.chatUtility.view setFrame:BTUtilityFrame];
...
-(void)showEmotions
{
    ...
    if (_bottomShowComponent & BTShowKeyboard)
    {
        ...
        _bottomShowComponent = (_bottomShowComponent & 0) | BTShowEmotion;
        [self.chatInputView.textView resignFirstResponder];
    ...
    else if (_bottomShowComponent & BTShowEmotion)
    {
        ...
        [self.chatInputView.textView resignFirstResponder];
        _bottomShowComponent = _bottomShowComponent & BTHideEmotion;
    }
    else if (_bottomShowComponent & BTShowUtility)
    {
        ...
        [self.chatUtility.view setFrame:BTComponentBottom];
        [self.emotions.view setFrame:BTEmotionFrame];
        _bottomShowComponent = (_bottomShowComponent & BTHideUtility) | BTShowEmotion;
    }
    else
    {
        //这是什么都没有显示，需用动画显示表情
        _bottomShowComponent = _bottomShowComponent | BTShowEmotion;
        [UIView animateWithDuration:0.25 animations:^{
            [self.emotions.view setFrame:BTEmotionFrame];
            [self.chatInputView setFrame:BTInputTopFrame];
        }];
...
-(void)handleWillShowKeyboard:(NSNotification *)notification
{
    ...
    _bottomShowComponent = _bottomShowComponent | BTShowKeyboard;
    [UIView animateWithDuration:0.25 animations:^{
...
-(void)handleWillHideKeyboard:(NSNotification *)notification
{
    ...
    keyboardRect = [notification.userInfo[UIKeyboardFrameEndUserInfoKey] CGRectValue];
    keyboardRect = [self.view convertRect:keyboardRect fromView:nil];
    _bottomShowComponent = _bottomShowComponent & BTHideKeyboard;
    if (_bottomShowComponent & BTShowUtility)
    {
        // 显示的是插件
        [UIView animateWithDuration:0.25 animations:^{
