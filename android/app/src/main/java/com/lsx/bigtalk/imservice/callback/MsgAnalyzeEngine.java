package com.lsx.bigtalk.imservice.callback;
...
public class MsgAnalyzeEngine {
    public static String analyzeMessageDisplay(String content){
        ...
        while (!originContent.isEmpty()) {
            int nStart = originContent.indexOf(MessageConstant.IMAGE_MSG_PREFIX);
            if (nStart < 0) {// 没有头
                break;
            } else {
                String subContentString = originContent.substring(nStart);
                int nEnd = subContentString.indexOf(MessageConstant.IMAGE_MSG_SUFFIX);
                if (nEnd < 0) {// 没有尾
                    ...
                    originContent = subContentString.substring(nEnd
                            + MessageConstant.IMAGE_MSG_SUFFIX.length());

                    if(!TextUtils.isEmpty(pre) || !TextUtils.isEmpty(originContent)){
                        finalRes = DBConstant.DISPLAY_FOR_RICH_TEXT;
                    }else{
    ...
    public static MessageEntity analyzeMessage(IMBaseDefine.MsgInfo msgInfo) {
       ...
       messageEntity.setContent(msgInfo.getMsgData().toStringUtf8());
         ...
         * 解密文本信息
         */
       String desMessage = new String(com.lsx.bigtalk.Security.getInstance().DecryptMsg(msgInfo.getMsgData().toStringUtf8()));
       ...
       if(!TextUtils.isEmpty(desMessage)){
           ...
           if(msgList.size()>1){
               // 混合消息
               RichTextMessageEntity mixMessage = new RichTextMessageEntity(msgList);
               return mixMessage;
    ...
    public static MessageEntity addMessage(MessageEntity msg,String strContent) {
        ...
        if (strContent.startsWith(MessageConstant.IMAGE_MSG_PREFIX)
            ...
            try {
                ImageMessageEntity imageMessage =  ImageMessageEntity.parseFromNet(msg);
                return imageMessage;
