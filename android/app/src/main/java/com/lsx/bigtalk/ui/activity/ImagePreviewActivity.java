...
public class ImagePreviewActivity extends Activity  {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		imageUrl = intent.getStringExtra(AppConstant.IntentConstant.KEY_AVATAR_URL);
		logger.d("imageUrl:%s", imageUrl);

		boolean isAvatar = intent.getBooleanExtra(AppConstant.IntentConstant.KEY_IS_IMAGE_CONTACT_AVATAR, false);
		...
		if (isAvatar) {
			IMUIHelper.setEntityImageViewAvatarNoDefaultPortrait(portraitView, imageUrl, AppConstant.DBConstant.SESSION_TYPE_SINGLE, 0);
		} else {
