...
import com.lsx.bigtalk.service.event.HistoryMsgRefreshEvent;
import com.lsx.bigtalk.service.service.LoadImageService;
...
public class IMMessageManager extends IMManager {
    ...
    private long getTimeoutTolerance(MessageEntity msg) {
        if (msg.getDisplayType() == AppConstant.DBConstant.SHOW_TYPE_IMAGE) {
            return 4 * 60 * 1000;
        }
        return 6 * 1000;
    }
    ...
    public void onEvent(MessageEvent event) {
        ...
        switch (type) {
            case IMAGE_UPLOAD_FAILURE:
            {
                ...
                imageMessage.setStatus(AppConstant.MessageConstant.MSG_FAILURE);
                btDb.insertOrUpdateMessage(imageMessage);
        ...
        imSocketManager.sendRequest(msgData, sid, cid, new PacketListener(getTimeoutTolerance(messageEntity)) {
            ...
            public void onSuccess(Object response) {
                try {
                    ...
                    btDb.insertOrUpdateMessage(messageEntity);
            ...
            public void onFailed() {
                logger.i("IMMessageManager#sendMessage#onFailed");
                messageEntity.setStatus(AppConstant.MessageConstant.MSG_FAILURE);
                btDb.insertOrUpdateMessage(messageEntity);
    ...
    public void onReceiveMessage(IMMessage.IMMsgData imMsgData) {
        ...
        receivedMessage.setStatus(AppConstant.MessageConstant.MSG_SUCCESS);

        btDb.insertOrUpdateMessage(receivedMessage);
	...
	public void sendText(TextMessageEntity textMessage) {
        ...
        BTDB.instance().insertOrUpdateMessage(textMessage);
	...
	public void sendVoice(AudioMessageEntity audioMessage) {
        ...
        BTDB.instance().insertOrUpdateMessage(audioMessage);
    ...
    public void sendImages(List<ImageMessageEntity> msgList) {
        ...
        BTDB.instance().batchInsertOrUpdateMessage(needDbList);
		...
		for (ImageMessageEntity msg : msgList) {
            ...
            switch (loadStatus) {
                ...
                {
                    msg.setLoadStatus(AppConstant.MessageConstant.IMAGE_LOADING);
                    Intent loadImageIntent = new Intent(ctx, LoadImageService.class);
                    loadImageIntent.putExtra(AppConstant.SysConstant.UPLOAD_IMAGE_INTENT_PARAMS, msg);
	...
	public void resendMessage(MessageEntity msgInfo) {
        ...
        if (!SequenceNumberMaker.getInstance().isFailure(msgInfo.getMsgId())) {
            msgInfo.setStatus(AppConstant.MessageConstant.MSG_SUCCESS);
            btDb.insertOrUpdateMessage(msgInfo);
    ...
    public List<MessageEntity> loadHistoryMsg(int pullTimes, String sessionKey, PeerEntity peerEntity) {
        ...
        int count = AppConstant.SysConstant.MSG_PAGE_SIZE;
    ...
    public List<MessageEntity> loadHistoryMsg(MessageEntity entity, int pullTimes){
        ...
        int cnt = AppConstant.SysConstant.MSG_PAGE_SIZE;
        return doLoadHistoryMsg(
                pullTimes,
    ...
    private List<MessageEntity> doLoadHistoryMsg(
        ...
        // 降序结果输出desc
        List<MessageEntity> listMsg = btDb.getHistoryMsg(sessionKey, lastMsgId, lastCreateTime, count);
        Log.i("LShuXin", "btDb.getHistoryMsg(" + sessionKey + ", " + lastMsgId + ", " + lastCreateTime + ", " + count + ")");
        // asyn task refresh
    ...
    public void updateMissedMsg(int sessionId, int sessionType, String chatKey, int msgIdEnd, int msgCnt) {
        ...
        List<Integer> msgIdList = btDb.refreshHistoryMsgId(chatKey, msgIdBeg, msgIdBeg);
    ...
    public void handleFetchMsgByIdResp(IMMessage.IMGetMsgByIdRsp resp) {
        ...
        btDb.batchInsertOrUpdateMessage(dbEntity);
    ...
    public  void fetchHistoryMsg(int sessionId, int sessionType, int lastMsgId, int cnt) {
     ...
     public void handleFetchHistoryMsgResp(IMMessage.IMGetMsgListRsp rsp) {
         ...
         if (!result.isEmpty()) {
             btDb.batchInsertOrUpdateMessage(result);
             MessageEvent event = new MessageEvent();
             event.setEvent(MessageEvent.Event.HISTORY_MSG_OBTAINED);
             triggerEvent(event);
         }
    ...
    private void onImageLoadSuccess(MessageEvent messageEvent) {
        ...
        imageMessageEntity.setStatus(AppConstant.MessageConstant.MSG_SUCCESS);
        imageMessageEntity.setLoadStatus(AppConstant.MessageConstant.IMAGE_LOADED_SUCCESS);
        btDb.insertOrUpdateMessage(imageMessageEntity);
        ...
        imageMessageEntity.setContent(AppConstant.MessageConstant.IMAGE_MSG_PREFIX
                + realImageUrl + AppConstant.MessageConstant.IMAGE_MSG_SUFFIX);
