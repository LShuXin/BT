...
public class ImageMessageEntity extends MessageEntity implements Serializable {
    ...
    public static ImageMessageEntity parseFromNet(MessageEntity entity) throws JSONException {
        ...
        if (strContent.startsWith(AppConstant.MessageConstant.IMAGE_MSG_PREFIX)
            ...
            imageMessage.setDisplayType(AppConstant.DBConstant.SHOW_TYPE_IMAGE);
            String imageUrl = strContent.substring(AppConstant.MessageConstant.IMAGE_MSG_PREFIX.length());
            imageUrl = imageUrl.substring(0, imageUrl.indexOf(AppConstant.MessageConstant.IMAGE_MSG_SUFFIX));

            JSONObject extraContent = new JSONObject();
            extraContent.put("path", "");
            extraContent.put("url", imageUrl);
            extraContent.put("loadStatus", AppConstant.MessageConstant.IMAGE_UNLOAD);
            ...
            imageMessage.setLoadStatus(AppConstant.MessageConstant.IMAGE_UNLOAD);
            imageMessage.setStatus(AppConstant.MessageConstant.MSG_SUCCESS);
            return imageMessage;
        } else {
    ...
    public static ImageMessageEntity parseFromDB(MessageEntity entity) {
        if (entity.getDisplayType() != AppConstant.DBConstant.SHOW_TYPE_IMAGE) {
        ...
        try {
            ...
            //todo temp solution
            if (loadStatus == AppConstant.MessageConstant.IMAGE_LOADING) {
                loadStatus = AppConstant.MessageConstant.IMAGE_UNLOAD;
            }
    ...
    public static ImageMessageEntity buildForSend(ImageItem item, UserEntity fromUser, PeerEntity peerEntity) {
        ...
        msg.setDisplayType(AppConstant.DBConstant.SHOW_TYPE_IMAGE);
        // content 自动生成的
        int peerType = peerEntity.getType();
        int msgType = peerType == AppConstant.DBConstant.SESSION_TYPE_GROUP ? AppConstant.DBConstant.MSG_TYPE_GROUP_TEXT :
                AppConstant.DBConstant.MSG_TYPE_SINGLE_TEXT;
        msg.setMsgType(msgType);

        msg.setStatus(AppConstant.MessageConstant.MSG_SENDING);
        msg.setLoadStatus(AppConstant.MessageConstant.IMAGE_UNLOAD);
    ...
    public byte[] getSendContent() {
        String sendContent = AppConstant.MessageConstant.IMAGE_MSG_PREFIX + url + AppConstant.MessageConstant.IMAGE_MSG_SUFFIX;
